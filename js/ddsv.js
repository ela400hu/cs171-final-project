...
divisions = [data[0]["Division of Court"]];   
ctypes= [data[0]["Type of Case"]];
dtypes=[data[0]["Type of Disposition"]];

//fill the unique values, they will be node names
// and aggregate the year values while we're in there

for(i=0;i<data.length;i++){
	if (dtypes.indexOf(data[i]["Type of Disposition"]) == -1) dtypes.push(data[i]["Type of Disposition"]) 
	if (ctypes.indexOf(data[i]["Type of Case"]) == -1) ctypes.push(data[i]["Type of Case"]) 
	if (divisions.indexOf(data[i]["Division of Court"]) == -1) divisions.push(data[i]["Division of Court"])
...
for(i=0;i<data.length;i++){
	dindex = dtypes.indexOf(data[i]["Type of Disposition"]) 
