...
DetailsVis = function(_parentElement, _whichData, _whichViz, _eventHandler){
    this.parentElement = _parentElement;
    this.whichData = _whichData;
    this.whichViz = _whichViz;
    this.eventHandler = _eventHandler;

		if (this.whichData == "0") { this.file = "dv_pop.csv" };
		if (this.whichData == "1") { this.file = "dv_fil_doc.csv" };
		if (this.whichData == "2") { this.file = "dv_fil_toc.csv" };
		if (this.whichData == "3") { this.file = "dv_dis_doc.csv" };
		if (this.whichData == "4") { this.file = "dv_dis_toc.csv" };
...
DetailsVis.prototype.onSelectionChange= function (selectionStart, selectionEnd){
	...
	d3.selectAll("#divmainhead svg").remove();
	d3.selectAll("#divmainbody svg").remove();
	//that.detVis();
}
...
DetailsVis.prototype.detVis = function () {
	...
	var margin = {top: 30, right: 40, bottom: 20, left: 30},
			...
			heighth = window.innerHeight*0.15 - margin.top - margin.bottom;
	...
	d3.csv("data/"+that.file, function(data) {
		...
		dimensions.forEach(function(dimension) {
			var getExtent = d3.extent(data, function(d) { return +d[dimension.name]; });
			if (getExtent[0] < allMin) { allMin = getExtent[0] };
			if (getExtent[1] > allMax) { allMax = getExtent[1] };
		});
